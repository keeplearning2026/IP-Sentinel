...
if command -v systemctl >/dev/null 2>&1; then
    ...
    systemctl disable --now ip-sentinel-runner.service ip-sentinel-runner.timer \
        ...
        ip-sentinel-data.service ip-sentinel-data.timer >/dev/null 2>&1
    systemctl disable --now ip-sentinel-keywords.service ip-sentinel-keywords.timer >/dev/null 2>&1 || true
    systemctl kill --signal=SIGKILL ip-sentinel-agent-daemon.service >/dev/null 2>&1 || true
    systemctl disable --now ip-sentinel-updater.service ip-sentinel-updater.timer \
        ip-sentinel-agent-daemon.service >/dev/null 2>&1 || true
